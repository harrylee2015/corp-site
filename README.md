...
> 类似猪八戒的信息收集与发布平台，支持身份驱动的用户中心、公司信息与项目管理、管理员审核、Excel 报表导出，适配手机浏览器。
...
- **用户中心**：登录后管理公司信息、发布项目，须完成企业认证后方可添加项目
- **分类导航**：两级分类树（6 个一级 + 悬停下拉二级），首页分段导航栏
- **信息发布（旧版）**：保留 posts 流程，与 projects 并行
- **审核机制**：信息/项目均需管理员审核通过后才能对外展示
...
│   ├── data/regions.go             # 省市区与地区 JSON 校验
│   ├── model/                      # user, company, project, category, post...
...
| categories | 两级分类树 |
| companies | 公司信息（每用户一条） |
| projects | 项目（含审核状态；金融字段可空） |
...
```
项目：pending → approved（首页展示）→ rejected
